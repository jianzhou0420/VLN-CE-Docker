"""
CMA (Cross-Modal Attention) adapter for VLN-CE policy server.

Handles loading the CMA model, managing RNN state, and transforming
...
class CMAAdapter(PolicyAdapter):
    """Adapter for CMA (Cross-Modal Attention) policy.

    Handles:
    - Loading CMA model from checkpoint
    ...
    def __init__(
        ...
        config_path: str = "vlnce_baselines/config/r2r_baselines/cma.yaml",
    ):
        """Initialize CMA adapter.

        Args:
            checkpoint_path: Path to CMA model checkpoint (.pth file)
            ...
            config_path: Path to CMA config YAML file
        """
        self.device = device
        ...
        # Load CMA model
        logger.info(f"Loading CMA model from {checkpoint_path}")
        self.policy = self._load_model()
        self.policy.eval()
        logger.info("CMA model loaded successfully")
    ...
    def _load_model(self):
        """Load CMA model from checkpoint.
    ...
    def _create_config(self):
        ...
        Returns:
            Config object with CMA settings
        """
        from vlnce_baselines.config.default import get_config

        # Load base config from CMA yaml
        config = get_config(self.config_path)

        # Override settings for inference
        config.defrost()

        # CMA-specific settings (bidirectional instruction encoder)
        config.MODEL.INSTRUCTION_ENCODER.bidirectional = True
        config.MODEL.INSTRUCTION_ENCODER.final_state_only = False
    ...
    def _create_observation_space(self) -> spaces.Dict:
        """Create observation space for CMA model.
