#!/usr/bin/env python3
"""
CMA Policy Server entry point.

Starts a WebSocket server that hosts a CMA (Cross-Modal Attention)
navigation policy for VLN-CE evaluation.

Usage:
    python vlnce_server/cma_server.py --port 8765
...
def main():
    parser = argparse.ArgumentParser(
        description="CMA Policy Server for VLN-CE Evaluation",
...
Examples:
    ...
    # Use specific GPU
    python vlnce_server/cma_server.py --port 8765 --gpu 1
        """,
    )
    parser.add_argument(
        "--host",
    ...
    parser.add_argument(
        "--checkpoint",
        type=str,
        default="data/checkpoints/CMA_PM_DA_Aug.pth",
        help="Path to CMA model checkpoint",
    )
    parser.add_argument(
        "--vocab",
        type=str,
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="vlnce_baselines/config/r2r_baselines/cma.yaml",
        help="Path to CMA config YAML file",
    )
    parser.add_argument(
        "--gpu",
        type=int,
    ...
    if not torch.cuda.is_available():
        logger.error("CUDA is not available. CMA server requires GPU.")
        return 1
    ...
    # Create CMA adapter
    logger.info("Initializing CMA adapter...")
    adapter = CMAAdapter(
